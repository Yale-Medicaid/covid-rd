...
def calc_maunscript_numbers():
    """
    Calculates numbers referenced in the manuscript text not found in figures and tables:
        - Total numbers of deaths, total and by Male/Female
        - Excess deaths between March - Decemeber for 65-74 years olds in NCHS and Datavant
    """
    ...
    for suff in ['tot', 'M', 'F']:
        df = pd.read_csv(directory+f'data/processed/private/RD_data_num_deaths_{suff}.txt',
                         sep='|')
        deaths = int(df.loc[df['year'].eq('2020') & df.age_rng.eq('(61, 69)'), 
                            f'num_deaths_{suff}'].sum())
        print(f'Deaths {suff} {deaths:,}')

    # Percentage change in deaths 
    dfdata = pd.read_csv(directory+'data/processed/private/excess_death_data.txt')
    # Weighted average so each death is weighted the same. [2:] becasue March - December
    excess = np.average(dfdata.loc[2:, 'excess_deaths'], weights=dfdata.loc[2:, 'deaths_1519'])
    print(f'Datavant excess deaths: {excess:.1f}')

    # NCHS
    # For NCHS go back to the raw data because we can use weekly granularity to get March - Dec. 
    file = 'Weekly_Counts_of_Deaths_by_Jurisdiction_and_Age.csv'

    df = pd.read_csv(directory+'data/raw/public/'+file)

    # Week 10 - 52 is March - December in 2020
    df = df[df['Age Group'].eq('65-74 years') 
           & df.Type.eq('Unweighted') 
           & df['Jurisdiction'].eq('United States')
           & df['Week'].between(10,52)]

    # Total deaths over time period 2015-2019, 2020, 2021
    df = df.groupby('Time Period')['Number of Deaths'].sum()

    # Divide deaths in 2020 by average in 2015-2019 and subtract 1 to get % excess
    excess = ((df/(df.loc['2015-2019']/5)).loc['2020']-1)*100
    print(f'NCHS excess deaths: {excess:.1f}')


if __name__ == '__main__':
    create_excess_death_figure()
    create_rd_figure()
    calc_maunscript_numbers()